...
int read_ref(struct Particle *set, int size, char *file_name);
// Main function
...
for(int i =0;i<n;i++){
diff.x = abs(locals[i].x - refs[i].x);
diff.y=abs(locals[i].y - refs[i].y);
diff.mass=abs(locals[i].mass-refs[i].mass);
diff.fx=abs(locals[i].fx-refs[i].fx);
diff.fy=abs(locals[i].fy - refs[i].fy);

if(diff.x>th || diff.y>th || diff.mass>th || diff.fx >th || diff.fy>th){
printf("Index=%d\tdiff.x=%f\tdiff.y=%f\tdiff.mass=%f\tdiff.fx=%f\tdiff.fy=%f\n",i,diff.x,diff.y,diff.mass,diff.fx,diff.fy);	
return false;
}
}
return true;
...
int read_ref(struct Particle *set, int size, char *file_name){
  ...
  // reading particle values
  for(int i; i<size; i++){
    fscanf(ifp, "%d\t%f\t%f\t%f\t%f\t%f",&ind, &set[i].x, &set[i].y, &set[i].mass, &set[i].fx, &set[i].fy);
}
